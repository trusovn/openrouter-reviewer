...
1. Choose the command:
   ...
   - For large context reviews, prefer `or-review diff --base HEAD` or `or-review files --file <small-set>` over broad file lists; if invalid structured output appears, retry with a narrower scope.
...
When a model fails with invalid structured output on a large context and another model succeeds, consume the successful partial report and assess the failed model as low usefulness. When every model fails on a large context, retry with a narrower `files` or `diff` scope before assuming the tool is broken.
